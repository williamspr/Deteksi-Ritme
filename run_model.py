...
def detect_rhythm(file_path, model_path='model/cnn_rhythm_model.h5', max_length=660):
    ...
    try:
        ...
        log_file.write(f"Spectrogram preprocessed for prediction, shape: {S_DB.shape}\n")
        log_file.flush()
        ...
        rhythm_class = int(prediction[0][0] > 0.5)  # Adjust this based on your binary classification
        rhythm_label = '4/4' if rhythm_class == 0 else '3/4'
        print(f"Prediction made, rhythm class: {rhythm_class}, rhythm label: {rhythm_label}")
        log_file.write(f"Prediction made, rhythm class: {rhythm_class}, rhythm label: {rhythm_label}\n")
        log_file.flush()
        ...
        log_file.write(f"Returning rhythm_label: {rhythm_label}, output_image: {output_image}\n")
        log_file.close()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 run_model.py <file_path>")
    else:
